<template>
  <q-page class="q-pa-sm">
    <div class="row items-center q-mb-sm">
      <div><div class="text-subtitle1 text-weight-bold">Almacenes</div><div class="text-caption text-grey-7">Revisiones del stock fÃ­sico de la tienda</div></div>
      <q-space/><q-btn v-if="can('Crear Almacenes')" dense unelevated color="primary" icon="add" label="Nueva revisiÃ³n" no-caps @click="openCreate"/>
    </div>

    <div class="kpi-row q-mb-sm">
      <q-card flat bordered class="kpi-card"><q-card-section class="q-pa-sm row items-center"><q-avatar icon="fact_check" color="orange-1" text-color="orange-9" size="38px"/><div class="q-ml-sm"><div class="text-caption text-grey-7">En revisiÃ³n</div><div class="text-h6 text-weight-bold">{{summary.en_revision}}</div></div></q-card-section></q-card>
    ...
    <q-card flat bordered>
      <q-card-section class="row q-col-gutter-sm q-pa-sm">
        <q-input v-model="filters.q" dense outlined clearable debounce="300" class="col-12 col-sm" placeholder="Buscar nÃºmero, usuario o descripciÃ³n" @update:model-value="reload"><template #prepend><q-icon name="search"/></template></q-input>
      ...
      <q-table flat dense :rows="rows" :columns="columns" row-key="id" :loading="loading" v-model:pagination="pagination" :rows-per-page-options="[20,50,100]" @request="onRequest">
        ...
        <template #body-cell-estado="p"><q-td :props="p"><q-badge :color="stateColor(p.row.estado)" :label="p.row.estado==='BORRADOR'?'EN REVISIÃ“N':p.row.estado"/></q-td></template>
        <template #body-cell-actions="p"><q-td :props="p">
          <q-btn v-if="p.row.estado==='BORRADOR'&&can('Editar Almacenes')" dense flat round color="primary" icon="fact_check" :to="`/almacenes/${p.row.id}`"><q-tooltip>Llenar la revisiÃ³n</q-tooltip></q-btn>
          ...
          <q-btn v-if="p.row.estado==='BORRADOR'&&can('Anular Almacenes')" dense flat round color="grey-7" icon="delete" @click="remove(p.row)"><q-tooltip>Eliminar revisiÃ³n</q-tooltip></q-btn>
        </q-td></template>
    ...
    <q-dialog v-model="createDialog">
      <q-card style="width:430px;max-width:94vw">
        <q-form @submit.prevent="create">
          <q-card-section class="row items-center q-py-sm bg-primary text-white"><q-avatar color="white" text-color="primary" icon="warehouse" size="32px"/><div class="q-ml-sm"><div class="text-subtitle1 text-weight-bold">Nueva revisiÃ³n de almacÃ©n</div><div class="text-caption">DespuÃ©s la llenan entre todos desde sus celulares</div></div></q-card-section>
          <q-card-section class="q-pa-md">
            <q-input v-model="form.descripcion" autofocus outlined dense label="DescripciÃ³n" placeholder="Ej. REVISIÃ“N GENERAL AGOSTO" maxlength="255" class="q-mb-sm"/>
            <q-input v-model="form.observacion" outlined dense autogrow label="ObservaciÃ³n (opcional)" maxlength="1000"/>
          </q-card-section>
          <q-separator/>
...
const stateOptions=[{label:'EN REVISIÃ“N',value:'BORRADOR'},{label:'APLICADO',value:'APLICADO'},{label:'ANULADO',value:'ANULADO'}]
const columns=[
  {name:'actions',label:'Acciones',align:'left'},
  {name:'numero',label:'NÃºmero',field:'numero',align:'left'},
  {name:'fecha',label:'Fecha',field:'fecha',align:'left'},
  {name:'descripcion',label:'DescripciÃ³n',field:r=>r.descripcion||'â€”',align:'left'},
  {name:'usuario_nombre',label:'CreÃ³',field:'usuario_nombre',align:'left'},
...
async function create(){
  ...
  try{
    ...
    proxy.$alert.success(`RevisiÃ³n ${data.numero} creada`)
    router.push(`/almacenes/${data.id}`)
  }catch(e){proxy.$alert.error(Object.values(e.response?.data?.errors||{})[0]?.[0]||e.response?.data?.message||'No se pudo crear la revisiÃ³n')}
  finally{creating.value=false}
}
function cancel(row){
  const aviso=row.estado==='APLICADO'?' El stock de cada producto volverÃ¡ al valor anterior.':''
  proxy.$alert.dialog(`Â¿Anular la revisiÃ³n ${row.numero}?${aviso}`).onOk(async()=>{
    try{await proxy.$axios.put(`/almacenes/${row.id}/anular`);proxy.$alert.success(`RevisiÃ³n ${row.numero} anulada`);load()}
    catch(e){proxy.$alert.error(e.response?.data?.message||'No se pudo anular la revisiÃ³n')}
  })
}
function remove(row){
  proxy.$alert.dialog(`Â¿Eliminar la revisiÃ³n ${row.numero}?`).onOk(async()=>{
    try{await proxy.$axios.delete(`/almacenes/${row.id}`);proxy.$alert.success('RevisiÃ³n eliminada');load()}
    catch(e){proxy.$alert.error(e.response?.data?.message||'No se pudo eliminar la revisiÃ³n')}
  })
}
onMounted(load)
</script>
