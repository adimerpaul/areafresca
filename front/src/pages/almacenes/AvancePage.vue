<template>
  <q-page class="q-pa-sm">
    <div class="row items-center q-mb-sm">
      <div>
        <div class="text-subtitle1 text-weight-bold">Avance de la revisiÃ³n {{almacen.numero}}
          <q-badge :color="stateColor" :label="almacen.estado==='BORRADOR'?'EN REVISIÃ“N':almacen.estado" class="q-ml-xs"/>
        </div>
        <div class="text-caption text-grey-7">
          <template v-if="almacen.estado==='APLICADO'">Aplicado por {{almacen.aplicado_por_nombre||'â€”'}} Â· {{formatDate(almacen.fecha_aplicado)}}</template>
    ...
    <q-card v-if="data.por_usuario?.length" flat bordered class="q-mb-sm">
      <q-card-section class="q-pa-sm row items-center">
        <div class="text-caption text-grey-7 q-mr-sm">QuiÃ©n contÃ³:</div>
    ...
    <q-card flat bordered>
      <q-card-section class="row items-center q-py-sm">
        <q-icon name="difference" color="primary" size="20px" class="q-mr-xs"/><b>Detalle de la revisiÃ³n</b>
        <q-space/>
        <q-checkbox v-model="onlyDifferences" dense size="sm" label="SÃ³lo diferencias" class="text-caption"/>
      </q-card-section>
      <q-separator/>
      <q-table flat dense :rows="rows" :columns="columns" row-key="id" :loading="loading" :pagination="{rowsPerPage:0}" hide-pagination>
        <template #body-cell-producto="p"><q-td :props="p"><b>{{p.row.nombre}}</b><div class="text-caption text-grey-7">{{p.row.codigo}} Â· {{p.row.unidad}}</div></q-td></template>
        ...
        <template #body-cell-lote="p"><q-td :props="p">
          <div v-if="p.row.conteos?.length" class="lot-cell">
            <q-badge v-for="lot in p.row.conteos" :key="lot.id" outline color="deep-orange"
                     :label="`${lot.lote||'sin lote'} Â· ${qty(lot.cantidad,p.row.unidad)}${lot.fecha_vencimiento?' Â· vence '+shortDate(lot.fecha_vencimiento):''}`"/>
          </div>
          <span v-else class="text-grey-6">â€”</span>
        </q-td></template>
        <template #body-cell-resultado="p"><q-td :props="p" class="text-right">
          <span v-if="p.row.stock_nuevo!==null&&p.row.stock_nuevo!==undefined" class="text-caption"><span class="text-grey-6">{{qty(p.row.stock_anterior,p.row.unidad)}}</span> â†’ <b>{{qty(p.row.stock_nuevo,p.row.unidad)}}</b></span>
          <span v-else class="text-caption text-grey-6">quedarÃ¡ en {{qty(p.row.cantidad,p.row.unidad)}}</span>
        </q-td></template>
        <template #no-data><div class="full-width text-center text-grey-6 q-py-xl"><q-icon name="inventory" size="42px"/><div>{{onlyDifferences?'No hay diferencias':'TodavÃ­a no se contÃ³ ningÃºn producto'}}</div></div></template>
      </q-table>
    </q-card>
    ...
    <q-dialog v-model="confirmDialog" :maximized="$q.screen.lt.sm">
      <q-card class="column no-wrap confirm-card">
        <q-card-section class="row items-center q-pa-sm confirm-head">
          ...
          <div class="q-ml-sm">
            <div class="text-subtitle1 text-weight-bold">Actualizar productos con la revisiÃ³n {{almacen.numero}}</div>
        ...
        <q-card-section class="q-pa-sm">
          <div class="confirm-kpis">
            ...
            <div class="confirm-kpi confirm-kpi--down"><div class="confirm-kpi__label">Faltantes</div><div class="confirm-kpi__value">âˆ’{{fmt(shortage.total)}}<span class="confirm-kpi__hint">{{shortage.count}} producto{{shortage.count===1?'':'s'}}</span></div></div>
        ...
        <q-card-section class="row items-center q-py-xs">
          ...
          <q-space/>
          <q-toggle v-model="confirmAll" dense size="sm" color="primary" label="Ver tambiÃ©n los que cuadran" class="text-caption"/>
        </q-card-section>
        <q-separator/>
        ...
        <div class="col scroll confirm-table">
          <q-markup-table dense flat separator="horizontal">
            <thead><tr>
              ...
              <th class="text-right">Diferencia</th><th class="text-right">QuedarÃ¡ en</th><th class="text-right">Valor Bs</th>
            </tr></thead>
            <tbody>
              <tr v-for="row in confirmRows" :key="row.id" :class="rowClass(row)">
                <td>
                  <div class="text-weight-medium">{{row.nombre}}</div>
                  <div class="text-caption text-grey-7">{{row.codigo}} Â· contÃ³ {{row.usuario_nombre||'â€”'}}</div>
                ...
                <td class="text-right"><span class="text-grey-6">{{qty(row.stock_actual,row.unidad)}}</span> â†’ <b>{{qty(row.cantidad,row.unidad)}}</b></td>
        ...
        <q-card-section class="q-py-xs bg-orange-1 text-orange-10 text-caption row items-center">
          ...
          Los {{Math.max(0,data.total_productos-data.revisados)}} productos que nadie contÃ³ no se modifican. Se guardarÃ¡ el valor anterior de cada producto para poder anular la revisiÃ³n.
        </q-card-section>
        <q-separator/>
        <q-card-actions align="right" class="q-pa-sm">
          <q-btn flat dense label="Cancelar" no-caps v-close-popup/>
          <q-btn dense unelevated color="positive" icon="published_with_changes" label="SÃ­, actualizar productos" no-caps :loading="applying" @click="runApply"/>
        </q-card-actions>
      </q-card>
    </q-dialog>
  </q-page>
...
// Resumen del diÃ¡logo de confirmaciÃ³n: sobrantes, faltantes y valor de cada lÃ­nea.
...
const lotSummary=row=>(row.conteos||[]).map(l=>`${l.lote||'sin lote'}: ${qty(l.cantidad,row.unidad)}${l.fecha_vencimiento?` (vence ${shortDate(l.fecha_vencimiento)})`:''}`).join(' Â· ')
const columns=[
  {name:'producto',label:'Producto',field:'nombre',align:'left'},
  {name:'usuario',label:'ContÃ³',field:r=>r.usuario_nombre||'â€”',align:'left'},
...
// BotÃ³n "Actualizar productos": primero se revisan las diferencias en el diÃ¡logo.
...
async function runApply(){
  ...
  try{
    ...
    proxy.$alert.success('Productos actualizados con la revisiÃ³n')
    await load()
